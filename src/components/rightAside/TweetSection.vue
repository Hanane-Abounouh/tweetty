<template>
  <div class="rounded-lg bg-blue-900 overflow-hidden shadow-lg m-4 mr-20">
    <div class="flex">
    ...
    <!-- Utilisation de v-for pour afficher les tendances -->
    <div v-for="(trend, index) in trends" :key="index" class="flex">
      <div class="flex-1">
        <p class="px-4 ml-2 mt-3 w-48 text-xs text-gray-400">{{ index + 1 }} . {{ trend.category }} . Trending</p>
        <h2 class="px-4 ml-2 w-48 font-bold text-white">{{ trend.hashtag }}</h2>
        <p class="px-4 ml-2 mb-3 w-48 text-xs text-gray-400">{{ trend.tweetCount }} Tweets</p>
      </div>
      <div class="flex-1 px-4 py-2 m-2">
...
export default {
  data() {
    return {
      trends: [] // Initialisation de la propriété trends
    };
  },
  created() {
    this.fetchTrends(); // Appel de la méthode fetchTrends lors de la création du composant
  },
  methods: {
    async fetchTrends() {
      try {
        ...
        this.trends = data; // Mise à jour de la propriété trends avec les données récupérées
      } catch (error) {
...
<style scoped>
/* Styles spécifiques à TweetSection.vue */
</style>
